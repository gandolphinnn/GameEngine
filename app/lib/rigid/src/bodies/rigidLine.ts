	...
	detectCollision(rigidBody: RigidBody) {
		switch (className(rigidBody)) {
			case 'RigidLine': {
				...
				const x1 = this.points[0].x, x2 = this.points[1].x, x3 = rigidLine.points[0].x, x4 = rigidLine.points[1].x;
				const y1 = this.points[0].y, y2 = this.points[1].y, y3 = rigidLine.points[0].y, y4 = rigidLine.points[1].y;
				const denom = (x1-x2)*(y3-y4) - (x3-x4)*(y1-y2);
				const t = ((x1-x3)*(y3-y4) - (x3-x4)*(y1-y3)) / denom;
				const u = ((x1-x3)*(y1-y2) - (x1-x2)*(y1-y3)) / denom;
				if (t >= 0 && t <= 1 && u >= 0 && u <= 1) {
			...
			case 'RigidPoly': {
				...
				let hitPoint: Coord = null;
				const x = rigidPoly.points[0].x, y = rigidPoly.points[0].y;
				let inside = false;
				for (let i = 0, j = rigidPoly.points.length - 1; i < rigidPoly.points.length; j = i++) {
					const xi = rigidPoly.points[i].x, yi = rigidPoly.points[i].y;
					const xj = rigidPoly.points[j].x, yj = rigidPoly.points[j].y;
